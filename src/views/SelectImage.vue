<template>
  <div class="box">
    ...
    <div v-for="(item,index) in SelectImage"
         :key="index"
         class="list">
      <div class="list_title"
           @click="button(rightdetails.SerialID)">
        <div>{{item.Name}}</div>
        <div>{{item.Count}}张图片</div>
      </div>
      <div v-for="(items,index) in item.List"
           :key="index"
           id="list_Img"
           @click="swiperImg(index)">
        <img :src="items.Url.split('{')[0]+'3'+items.Url.split('{')[1].split('}')[1]"
             :alt="items.Url" />
      </div>
      <SwiperImg :flag="flag" />
    </div>
